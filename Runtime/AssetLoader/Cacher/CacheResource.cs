...
{
    internal class  CacheResource : AssetCache<ResourcePack>, ISingleton
    {
        public override bool HasInCache(string assetName)
        {
            return this.m_Cacher.ContainsKey(assetName);
        }
        ...
        public override ResourcePack GetFromCache(string assetName)
        {
            ...
            {
                if (this.m_Cacher.TryGetValue(assetName, out ResourcePack pack))
                    return pack;
            }
        ...
        public async UniTask PreloadAssetAsync<T>(string[] assetNames, Progression progression, byte maxRetryCount) where T : Object
        {
            ...
            for (int i = 0; i < assetNames.Length; i++)
            {
                ...
                if (this.TryGetLoadingTask(assetName, out var existingTask))
                {
                    ...
                    {
                        Debug.LogError($"【Preload】Asset: {assetName} failed: {ex.Message}");
                    }
                    continue;
                }
                ...
                if (this.HasInCache(assetName))
                {
                    this.currentCount++;
                    progression?.Invoke(this.currentCount / this.totalCount, this.currentCount, this.totalCount);
                    Debug.LogWarning($"【Preload】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: [{assetName}] already preloaded!!!");
                    continue;
                }
        ...
        private async UniTask _PreloadAssetCoreAsync<T>(string assetName, Progression progression, byte maxRetryCount) where T : Object
        {
            ...
            if (loaded)
            {
                ...
                if (!this.HasInCache(assetName))
                {
                    ...
                    Debug.Log($"【Preload】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: {assetName}");
                }
            }
            else
            {
                ...
                if (this.GetRetryCounter(assetName) == null)
                {
                    ...
                    Debug.Log($"【Preload】 => << {nameof(CacheResource)} >> Asset: {assetName} start doing retry. Max retry count: {maxRetryCount}");
                }
                else
                {
                    Debug.Log($"【Preload】 => << {nameof(CacheResource)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                }
        ...
        public async UniTask<T> LoadAssetAsync<T>(string assetName, Progression progression, byte maxRetryCount) where T : Object
        {
            ...
            if (this.TryGetLoadingTask(assetName, out var existingTask))
            {
                ...
                if (asset != null)
                {
                    ...
                    {
                        pack.AddRef();
                        Debug.Log($"【Load Shared】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: {assetName}, ref: {pack.refCount}");
                    }
                }
                return asset;
            }
            ...
            if (pack != null)
            {
                ...
                {
                    pack.AddRef();
                    Debug.Log($"【Load】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: {assetName}, ref: {pack.refCount}");
                }
                return cachedAsset;
            }
        ...
        private async UniTask<T> _LoadAssetCoreAsync<T>(string assetName, Progression progression, byte maxRetryCount) where T : Object
        {
            ...
            else
            {
                ...
                {
                    this.StartRetryCounter(assetName, maxRetryCount);
                    Debug.Log($"【Load】 => << {nameof(CacheResource)} >> Asset: {assetName} start doing retry. Max retry count: {maxRetryCount}");
                }
                else
                {
                    Debug.Log($"【Load】 => << {nameof(CacheResource)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                }
        ...
        private void _UnloadAssetCore(string assetName, bool forceUnload, ProcessType processType)
        {
            ...
            try
            {
                // 引用計數--
                pack.DelRef();
                Debug.Log($"【Unload】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: {assetName}, ref: {this.m_Cacher.TryGetValue(assetName, out var v)} {v?.refCount}");
                ...
                if (shouldUnload)
                {
                    ...
                    Debug.Log($"【Unload Completes】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}, asset: {assetName}");
                }
            }
            finally
            {
        ...
        public void ReleaseAssets()
        {
            ...
            Debug.Log($"【Release All】 => Current << {nameof(CacheResource)} >> Cache Count: {this.count}");
        }
        #endregion
    }
}