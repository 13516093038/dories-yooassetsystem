...
{
    internal class CacheBundle : AssetCache<BundlePack>, ISingleton
    {
        /// <summary>
        /// 緩存 Scene BundlePack
        /// </summary>
        ...
        {
            return this.m_Cacher.ContainsKey(assetName) || this._additiveSceneCounter.ContainsKey(assetName);
        }
        ...
        public override BundlePack GetFromCache(string assetName)
        {
            ...
            {
                if (this.m_Cacher.TryGetValue(assetName, out BundlePack pack))
                    return pack;
            }
        ...
        public async UniTask<BundlePack> LoadSceneAsync(string packageName, string assetName, LoadSceneMode loadSceneMode, LocalPhysicsMode localPhysicsMode, bool activateOnLoad, uint priority, Progression progression)
        {
                ...
                this.GetRetryCounter(assetName).IsOutOfRetries())
            {
                ...
                Debug.Log($"Asset: {assetName} Load failed and cannot retry anymore!!! Please to check asset is existing.");
                return null;
            }
            ...
            {
                Debug.Log($"Scene: {assetName} is loading, waiting for existing task...");
                                ...
                                case LoadSceneMode.Additive:
                                    {
                                        ...
                                        if (!this._additiveSceneCounter.ContainsKey(assetName))
                                        {
                                            ...
                                            Debug.Log($"【Load Scene Additive】 => << {nameof(CacheBundle)} >> scene: {key}");
                                        }
                                        else
                                        {
                                            var count = ++this._additiveSceneCounter[assetName];
            ...
            if (!suspendLoaded)
            {
                if (!loaded)
                {
                    ...
                    if (this.GetRetryCounter(assetName) == null)
                    {
                        ...
                        Debug.Log($"【Load Scene】 => << {nameof(CacheBundle)} >> Asset: {assetName} start doing retry. Max retry count: {maxRetryCount}");
                    }
                    else
                    {
                        Debug.Log(
                            $"【Load Scene】 => << {nameof(CacheBundle)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                    }
        ...
        public BundlePack LoadScene(string packageName, string assetName, LoadSceneMode loadSceneMode, LocalPhysicsMode localPhysicsMode, Progression progression)
        {
            ...
            if (!loaded)
            {
                ...
                {
                    Debug.Log($"【Load Scene】 => << {nameof(CacheBundle)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                }
        ...
        private void _UnloadSceneCore(string assetName, bool recursively, ProcessType processType)
        {
                    ...
                    if (recursively)
                    {
                        ...
                        for (int topCount = this._additiveSceneCounter[assetName]; topCount >= 1; --topCount)
                        {
                            ...
                            if (this._additiveScenes.ContainsKey(key))
                            {
                                ...
                                if (package == null)
                                    package = GetPackage(pack.packageName);
                                if (pack.IsSceneOperationHandle())
                                {
                                    pack.UnloadScene();
                                    this._additiveScenes[key] = null;
                                    this._additiveScenes.Remove(key);
                                    Debug.Log($"【Unload Additive Scene】 => << {nameof(CacheBundle)} >> scene: {key}, count: {topCount}");
                                }
                            }
                        }
                        ...
                        Debug.Log($"【Unload Additive Scene Completes】 => << {nameof(CacheBundle)} >> sceneName: {assetName}, recursively: {recursively}");
                    }
                    else
                    {
            ...
            else Debug.Log($"【Unload Scene Invalid】 => << {nameof(CacheBundle)} >> sceneName: {assetName} maybe not Additive or is Single");
        }
        ...
        public void ReleaseScenes()
        {
            ...
            foreach (var assetName in this._additiveSceneCounter.Keys.ToArray())
            {
                if (this.HasInCache(assetName))
                {
                    ...
                    var package = GetPackage(pack.packageName);
            ...
            Debug.Log($"【Release All Scenes (Addtive)】 => Current << {nameof(CacheBundle)} >> Additive Scene Cache Count: {this._additiveSceneCounter.Count}");
        }
        #endregion
        ...
        public async UniTask PreloadRawFileAsync(string packageName, string[] assetNames, uint priority, Progression progression, byte maxRetryCount)
        {
            ...
            for (int i = 0; i < assetNames.Length; i++)
            {
                ...
                if (this.TryGetLoadingTask(assetName, out var existingTask))
                {
                    Debug.Log($"RawFile: {assetName} is loading, waiting for existing task...");
                    ...
                    {
                        Debug.Log($"【Preload】Asset: {assetName} failed: {ex.Message}");
                    }
                    continue;
                }
                ...
                if (this.HasInCache(assetName))
                {
                    ...
                    if (pack.IsRawFileOperationHandleValid())
                    {
                        this.currentCount++;
                        progression?.Invoke(this.currentCount / this.totalCount, this.currentCount, this.totalCount);
                        Debug.Log($"【Preload】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: [{assetName}] already preloaded!!!");
                    }
                    continue;
                }
        ...
        private async UniTask _PreloadRawFileCoreAsync(string packageName, string assetName, uint priority, Progression progression, byte maxRetryCount)
        {
            ...
            if (loaded)
            {
                ...
                if (!this.HasInCache(assetName))
                {
                    this.m_Cacher.Add(assetName, pack);
                    Debug.Log($"【Preload】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: {assetName}");
                }
            }
            else
            {
                ...
                {
                    this.StartRetryCounter(assetName, maxRetryCount);
                    Debug.Log($"【Preload】 => << {nameof(CacheBundle)} >> Asset: {assetName} start doing retry. Max retry count: {maxRetryCount}");
                }
                else
                {
                    Debug.Log($"【Preload】 => << {nameof(CacheBundle)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                }
        ...
        public void PreloadRawFile(string packageName, string[] assetNames, Progression progression, byte maxRetryCount)
        {
            ...
            for (int i = 0; i < assetNames.Length; i++)
            {
                ...
                {
                    this.StopRetryCounter(assetName);
                    Debug.LogWarning($"Asset: {assetName} Load failed and cannot retry anymore!!! Please to check asset is existing.");
                    continue;
                }
                ...
                if (this.HasInCache(assetName))
                {
                    ...
                    if (pack.IsRawFileOperationHandleValid())
                    {
                        this.currentCount++;
                        progression?.Invoke(this.currentCount / this.totalCount, this.currentCount, this.totalCount);
                        Debug.LogWarning($"【Preload】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: [{assetName}] already preloaded!!!");
                    }
                    continue;
                }
        ...
        public async UniTask<T> LoadRawFileAsync<T>(string packageName, string assetName, uint priority, Progression progression, byte maxRetryCount)
        {
            ...
            if (pack != null)
            {
                ...
                {
                    pack.AddRef();
                    Debug.Log($"【Load】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: {assetName}, ref: {pack.refCount}");
                    return (T)data;
                }
            }
        ...
        private async UniTask<T> _LoadRawFileCoreAsync<T>(string packageName, string assetName, uint priority, Progression progression, byte maxRetryCount)
        {
            ...
            else
            {
                ...
                {
                    this.StartRetryCounter(assetName, maxRetryCount);
                    Debug.Log($"【Load】 => << {nameof(CacheBundle)} >> Asset: {assetName} start doing retry. Max retry count: {maxRetryCount}");
                }
                else
                {
                    Debug.Log($"【Load】 => << {nameof(CacheBundle)} >> Asset: {assetName} doing retry. Remaining retry count: {this.GetRetryCounter(assetName).retryCount}, Max retry count: {maxRetryCount}");
                }
        ...
        public async UniTask PreloadAssetAsync<T>(string packageName, string[] assetNames, uint priority, Progression progression, byte maxRetryCount) where T : Object
        {
            ...
            for (int i = 0; i < assetNames.Length; i++)
            {
                ...
                if (this.TryGetLoadingTask(assetName, out var existingTask))
                {
                    Debug.Log($"Asset: {assetName} is loading, waiting for existing task...");
        ...
        private void _UnloadCore(string assetName, bool forceUnload, ProcessType processType)
        {
            if (!this.HasInCache(assetName))
            {
                Debug.LogWarning($"【Unload】 Asset: {assetName} not in cache, skipping.");
                return;
            }
                ...
                pack.IsAssetOperationHandle())
            {
                ...
                try
                {
                    // 引用計數--
                    pack.DelRef();
                    Debug.Log($"【Unload】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: {assetName}, ref: {pack.refCount}");
                    ...
                    if (shouldUnload)
                    {
                        ...
                        this.m_Cacher[assetName] = null;
                        this.m_Cacher.Remove(assetName);

                        var package = GetPackage(packageName);
                        package?.TryUnloadUnusedAsset(assetName, _DEFAULT_LOOP_COUNT);

                        Debug.Log($"【Unload Completes】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: {assetName}");
                    }
                }
                finally
                {
            ...
            {
                Debug.Log($"【Unload Type Error】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}, asset: {assetName}, ref: {pack.refCount}");
            }
        }
        ...
        private void _ReleaseCore(ProcessType processType)
        {
            ...
            // 強制釋放緩存與資源
            foreach (var assetName in this.m_Cacher.Keys.ToArray())
            {
                if (this.HasInCache(assetName))
                {
                    BundlePack pack = this.GetFromCache(assetName);
            ...
            Debug.Log($"【Release All {type}】 => Current << {nameof(CacheBundle)} >> Cache Count: {this.count}");
        }
        #endregion
